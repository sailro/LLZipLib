...
public class CentralDirectoryFooter : Block
{
	...
	{
		get => ZipArchive?.StringConverter.GetString(CommentBuffer, StringConverterContext.Comment) ?? throw new InvalidOperationException("this footer must be linked to a ZipArchive");
		set => CommentBuffer = ZipArchive?.StringConverter.GetBytes(value, StringConverterContext.Comment) ?? throw new InvalidOperationException("this footer must be linked to a ZipArchive");
	}
	...
	{
		return 3 * sizeof(uint) + 5 * sizeof(ushort) + CommentBuffer.Length;
	}
	...
	internal void Write(BinaryWriter writer)
	{
		...
		var zip = ZipArchive ?? throw new InvalidOperationException("this footer must be linked to a ZipArchive");
